class UsersController < ApplicationController
  ...
  def chat
    ...
    # @messages = Message.find_by_sql "SELECT temp.message as 'message', temp.from_user as 'from_user', users.name as 'to_user', temp.created_at as 'created_at' FROM (SELECT message, users.name as 'from_user', user_id as 'from_id', to_id, messages.created_at FROM messages, users where messages.user_id = users.id) as 'temp',users where temp.to_id = users.id AND (temp.to_id ="+session[:id_other].to_s+" OR temp.from_id = "+session[:id_other].to_s+") ORDER BY created_at DESC"
